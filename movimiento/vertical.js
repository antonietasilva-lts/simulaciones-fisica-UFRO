...
(() => {
  ...
  function state(t){
    ...
    return {
      ...m,t,
      y:Math.max(0,m.y0+m.v0*t-.5*G*t*t),
      vy:m.v0-G*t,
      ay:-G
    };
  }
  ...
  function redraw(t){
    ...
    $("vertY0Label").textContent=F(s.y0)+" m";
    $("vertV0Label").textContent=F(s.v0)+" m/s";
    ...
    for(let i=0;i<=N;i++){
      ...
      xs.push(tt);ys.push(ss.y);vs.push(ss.vy);as.push(-G);
    }
