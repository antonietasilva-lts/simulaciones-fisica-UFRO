...
if ($('circular')) {
  ...
  function drawScene(state,p,stateB=null,pB=null){
    ...
    const showComponents=$('c-vectors').checked;
    const showResultant=$('c-total-acc').checked;
    const showAccelComponents=showComponents || showResultant;
    ...
    /* Aceleraciones: una única escala gráfica para que la suma visual sea correcta */
    const ac=state.centripetalAcceleration;
    const at=state.tangentialAcceleration;
    const maxComp=Math.max(ac,Math.abs(at),Math.hypot(ac,at),0.001);
    const aScale=Math.min(70/maxComp,12);

    const acVX=rInX*ac*aScale, acVY=rInY*ac*aScale;
    const atVX=tBaseX*at*aScale, atVY=tBaseY*at*aScale;
    const aVX=acVX+atVX, aVY=acVY+atVY;

    if(showAccelComponents){
      if(ac>.0001){
        arrow(svg,x,y,x+acVX,y+acVY,'#6c47d7',3.2);
        textNode(svg,x+acVX+6,y+acVY-5,'aᶜ','c-ac-label');
      }
      if(Math.abs(at)>.0001){
        arrow(svg,x,y,x+atVX,y+atVY,'#e46e62',3.5);
        textNode(svg,x+atVX+6,y+atVY-5,'aᵗ','c-at-label');
      }
    }

    if(showResultant && Math.hypot(aVX,aVY)>.0001){
      /* La resultante se dibuja como diagonal del paralelogramo a_c + a_t */
      line(svg,x+acVX,y+acVY,x+aVX,y+aVY,'#b9becd',1.4,'4 4');
      line(svg,x+atVX,y+atVY,x+aVX,y+aVY,'#b9becd',1.4,'4 4');
      arrow(svg,x,y,x+aVX,y+aVY,'#ffffff',8);
      arrow(svg,x,y,x+aVX,y+aVY,'#111827',4.5);
      textNode(svg,x+aVX+8,y+aVY-7,'a','c-total-label');
    }
  ...
  function setMode(next){mode=next;const isMCUA=mode==='mcua';$('c-mode-mcu').classList.toggle('selected',!isMCUA);$('c-mode-mcua').classList.toggle('selected',isMCUA);$('c-mode-mcu').setAttribute('aria-pressed',String(!isMCUA));$('c-mode-mcua').setAttribute('aria-pressed',String(isMCUA));$('c-alpha-control').classList.toggle('disabled',!isMCUA);$('c-alpha').disabled=!isMCUA;$('c-alpha-b-row').classList.toggle('disabled',!isMCUA);$('c-alpha-b').disabled=!isMCUA;
const totalToggle=$('c-total-acc'); totalToggle.disabled=!isMCUA; totalToggle.closest('label').classList.toggle('disabled',!isMCUA); if(!isMCUA) totalToggle.checked=false; else totalToggle.checked=true;
$('c-mode-description').textContent=isMCUA?'Aceleración angular constante; se muestran aᶜ, aᵗ y su resultante a.':'Velocidad angular constante; α = 0 y por tanto aᵗ = 0.';$('c-equation').textContent=isMCUA?'θ(t) = θ₀ + ω₀t + ½αt²':'θ(t) = θ₀ + ω₀t';$('c-omega-equation').textContent=isMCUA?'ω(t) = ω₀ + αt':'ω(t) = ω₀';update()}
