...
if ($('circular')) {
  ...
  function drawScene(state,p,stateB=null,pB=null){
    ...
    const arcR=38,endX=cx+arcR*Math.cos(angle),endY=cy+arcR*Math.sin(angle);
    const normalized=((state.theta%TAU)+TAU)%TAU, sweep=state.theta>=0?0:1;
    if(normalized>.03){const path=el('path',{d:`M ${cx+arcR} ${cy} A ${arcR} ${arcR} 0 ${normalized>Math.PI?1:0} ${sweep} ${endX} ${endY}`,fill:'none',stroke:'#9b84c0','stroke-width':2});svg.append(path)}
    textNode(svg,cx+46,cy-17,'θ','c-angle-label');
    ...
    if($('c-vectors').checked){
      const sign=state.omega===0?1:Math.sign(state.omega),tx=-Math.sin(angle)*sign,ty=Math.cos(angle)*sign;
      const vLen=Math.min(92,28+state.tangentialSpeed*7);arrow(svg,x,y,x+tx*vLen,y+ty*vLen,'#138f91',3);
      const acLen=Math.min(88,20+state.centripetalAcceleration*2.5);arrow(svg,x,y,x+(cx-x)/rr*acLen,y+(cy-y)/rr*acLen,'#6c47d7',3);
      if(Math.abs(state.tangentialAcceleration)>.01){const atSign=Math.sign(state.tangentialAcceleration)*(state.omega===0?1:sign),atLen=Math.min(75,24+Math.abs(state.tangentialAcceleration)*7);arrow(svg,x,y,x+(-Math.sin(angle))*atSign*atLen,y+Math.cos(angle)*atSign*atLen,'#e46e62',3)}
    }
