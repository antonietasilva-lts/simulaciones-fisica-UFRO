...
if ($('circular')) {
  ...
  function drawScene(state,p,stateB=null,pB=null){
    ...
    const showAccelComponents=showComponents;
    ...
    const maxComp=Math.max(ac,Math.abs(at),0.001);
...
$('c-mode-description').textContent=isMCUA?'Aceleración angular constante; se muestran las componentes aᶜ y aᵗ.':'Velocidad angular constante; α = 0 y por tanto aᵗ = 0.';$('c-equation').textContent=isMCUA?'θ(t) = θ₀ + ω₀t + ½αt²':'θ(t) = θ₀ + ω₀t';$('c-omega-equation').textContent=isMCUA?'ω(t) = ω₀ + αt':'ω(t) = ω₀';update()}
  ...
  $('c-vectors').onchange=update;$('c-time').oninput=e=>{currentTime=Number(e.target.value);playing=false;update()};$('c-play').onclick=()=>{if(currentTime>=duration())currentTime=0;playing=!playing;update()};$('c-reset').onclick=()=>{currentTime=0;playing=false;update()};
