...
if ($('circular')) {
  ...
  function drawScene(state,p,stateB=null,pB=null){
    ...
    if(showResultant && Math.abs(state.tangentialAcceleration)>.01){
      ...
      const totalLen=Math.min(112,34+amag*3.2);
      arrow(svg,x,y,x+(ax/amag)*totalLen,y+(ay/amag)*totalLen,'#202637',4.2);
      textNode(svg,x+(ax/amag)*totalLen+7,y+(ay/amag)*totalLen-6,'a','c-total-label');
    }
...
const totalToggle=$('c-total-acc'); totalToggle.disabled=!isMCUA; totalToggle.closest('label').classList.toggle('disabled',!isMCUA); if(!isMCUA) totalToggle.checked=false;
