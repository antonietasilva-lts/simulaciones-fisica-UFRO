...
if ($('circular')) {
  ...
  function drawScene(state,p,stateB=null,pB=null){
    ...
    if(showAccelComponents){
      ...
      textNode(svg,x+rInX*acLen+6,y+rInY*acLen-5,'aᶜ','c-ac-label');

      if(Math.abs(state.tangentialAcceleration)>.01){
        const atSign=Math.sign(state.tangentialAcceleration);
        const atLen=Math.min(82,26+Math.abs(state.tangentialAcceleration)*8);
        arrow(svg,x,y,x+tBaseX*atSign*atLen,y+tBaseY*atSign*atLen,'#e46e62',3.5);
        textNode(svg,x+tBaseX*atSign*atLen+6,y+tBaseY*atSign*atLen-5,'aᵗ','c-at-label');
      }
    }
    ...
    if(showResultant){
      ...
      if(amag>.0001){
        ...
        const totalLen=Math.max(72,Math.min(132,44+amag*3.8));
        /* halo blanco para que la resultante no se pierda sobre a_c o a_t */
        arrow(svg,x,y,x+ux*totalLen,y+uy*totalLen,'#ffffff',8);
        arrow(svg,x,y,x+ux*totalLen,y+uy*totalLen,'#111827',4.5);
        textNode(svg,x+ux*totalLen+9,y+uy*totalLen-7,'a','c-total-label');
      }
    }
